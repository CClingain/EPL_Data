   ...
   sidebarLayout(
      sidebarPanel(
         selectizeInput(inputId = "x",
                     label = "Team 1",
                     c( "Arsenal" = 1, "Bournemouth" = 2,"Brighton and Hove Albion" =3,"Burnley" =4,"Chelsea"=5,"Crystal Palace"=6,
...
server <- function(input, output,session) {
  ...
  PointsTable.long<- reshape(PointsTable[,2:31], varying=c("Week01","Week02", "Week03", "Week04", "Week05", "Week06", "Week07", "Week08","Week09", "Week10", "Week11", "Week12","Week13", "Week14", "Week15", "Week16", "Week17", "Week18","Week19", "Week20", "Week21", "Week22", "Week23", "Week24",
   ...
   output$plot <- renderPlotly({
     ...
     plot_ly(PointsTable.long,x = ~data1$Time, y = ~data1$Week, name =data1$Teams, type = 'scatter', mode = 'lines') %>%
       add_trace(y = ~data2$Week, name = data2$Teams, mode = 'lines') %>%
